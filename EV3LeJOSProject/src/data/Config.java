...
public class Config {
    ...
    public synchronized void setDirection(String direction) {
        try {
            setDirection(Integer.parseInt(direction));
        } catch (NumberFormatException e) {
            System.out.println("Failed to parse direction: " + e.getMessage());
        }
    }
    ...
    public synchronized void setSpeed(String speed) {
        try {
            ...
            System.out.println("Failed to parse speed: " + e.getMessage());
        }
    }
    ...
    public synchronized void setTurn(String turn) {
        try {
            setTurn(Integer.parseInt(turn));
        } catch (NumberFormatException e) {
            System.out.println("Failed to parse turn: " + e.getMessage());
        }
    }
    ...
    public synchronized void setRun(int run) {
        this.run = run;
    }
    public synchronized void setRun(String run) {
        try {
            setTurn(Integer.parseInt(run));
        } catch (NumberFormatException e) {
            System.out.println("Failed to parse run: " + e.getMessage());
        }
    }
    
    public String toJsonString() {
        // Convert the configuration data to a JSON string
        return "{\"speed\":" + speed + ",\"direction\":" + direction + ",\"running\":" + run + ",\"turn\":" + turn + "}";
    }
}